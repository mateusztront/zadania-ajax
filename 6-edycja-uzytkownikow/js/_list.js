...
export function getUserElementHTML({name, surname, email}) {
    return `
...
export async function loadUserList(ul) {
    ul.innerHTML = "";

    await apiLoadData().then(res => {
        res.forEach(user => {
            const {first_name : name, last_name : surname, email, id} = user;
            const li = document.createElement('li');
            li.dataset.id = id;
            li.innerHTML = getUserElementHTML({name, surname, email});
            ul.prepend(li);
        })
    }).catch(err => {
        console.log(err);
